...
export class LineupComponent implements OnInit, OnDestroy {
  ...
  error;
  tag;
  allRosterslistCopyCopy = [];





  @ViewChild("playerList", { static: true }) playerList: ModalDirective;
  @ViewChild("formationModal", { static: true }) formationModal: ModalDirective;
  @ViewChild("selectCaptain", { static: true }) selectCaptain: ModalDirective;
  @ViewChild("joinGameConfirmModal", { static: true }) joinGameConfirmModal: ModalDirective;
  @ViewChild("captain", { static: true }) captain: ElementRef;
  @ViewChild("viceCaptain", { static: true }) viceCaptain: ElementRef;
  @ViewChild("team_name", { static: true }) team_name: ElementRef;
  @ViewChild("def1", { static: false }) def1: ElementRef;
  @ViewChild("def2", { static: false }) def2: ElementRef;
  @ViewChild("def3", { static: false }) def3: ElementRef;
  @ViewChild("def4", { static: false }) def4: ElementRef;
  @ViewChild("def5", { static: false }) def5: ElementRef;
  // for fwdfielders
  @ViewChild("mf1", { static: false }) mf1: ElementRef;
  @ViewChild("mf2", { static: false }) mf2: ElementRef;
  @ViewChild("mf3", { static: false }) mf3: ElementRef;
  @ViewChild("mf4", { static: false }) mf4: ElementRef;
  @ViewChild("mf5", { static: false }) mf5: ElementRef;
  // forwards
  @ViewChild("fwd1", { static: false }) fwd1: ElementRef;
  @ViewChild("fwd2", { static: false }) fwd2: ElementRef;
  @ViewChild("fwd3", { static: false }) fwd3: ElementRef;
  @ViewChild("fwd4", { static: false }) fwd4: ElementRef;
  @ViewChild("fwd5", { static: false }) fwd5: ElementRef;
  searchPlayerForm;
  confirmJoinForm;
  searchBoxForm;
  // message = '' ;
  ...
  ngOnInit() {
    ...
    this.playerSelectForm = new FormGroup({
      ...
      .subscribe(
        response => {
          if (response["response_code"] === 200) {
            ...
            //console.log(this.allRosterslist[0]);
  ...
  resetSelectorFilter(arg?) {

    this.playerSelectForm.reset();
    this.allRosterslistCopyCopy = [];
    this.playerList.hide();

  }
  searchfilter(query, arg?) {

    let quer = this.playerSelectForm.value.team;

    if (arg === 'selector') {
      this.allRosterslistCopyCopy = this.allRosterslistCopy.filter(data => data.team_league_id == quer);
    }
    if (arg === 'search') {

      if (this.allRosterslistCopyCopy.length > 0) {

        this.allRosterslistCopyCopy = (query) ?
          this.allRosterslistCopyCopy.filter(data => (
            data.first_name.toLowerCase().includes(query.toLowerCase()) ? data.first_name.toLowerCase().includes(query.toLowerCase())
              : (data.last_name.toLowerCase().includes(query.toLowerCase())) ? data.last_name.toLowerCase().includes(query.toLowerCase())
                : data.nick_name.toLowerCase().includes(query.toLowerCase())
          )
          ) : this.allRosterslistCopyCopy;

      } else {
        // this.resetSelectorFilter(true);
        this.allRosterslistCopyCopy = (query) ?
          this.allRosterslistCopy.filter(data => (
            data.first_name.toLowerCase().includes(query.toLowerCase()) ? data.first_name.toLowerCase().includes(query.toLowerCase())
              : (data.last_name.toLowerCase().includes(query.toLowerCase())) ? data.last_name.toLowerCase().includes(query.toLowerCase())
                : data.nick_name.toLowerCase().includes(query.toLowerCase())
          )
  ...
  joinGameModals(contest, lineup, lineupList, isTurbo) {
    ...
    const entryFee = contest ? Number(contest.entry_fee) : '';
    const param = {
      ...
      .subscribe(
        response => {
          ...
          if (
            ...
            this.error = 'You do not have enough cash to join this contest';
            this.tag = 'danger';
          } else if (
            etry_free > point_balance &&
            ((contest.prize_type === '2' || contest.prize_type === '3') && contest.contest_access_type !== '3')
          ) {
            // Condition for coin system
            // show notEnoughCashModal \\
            this.error = 'You do not have enough coin to join this contest';
            this.tag = 'danger';
          } else {
            if (etry_free > token && contest.contest_access_type === '3') {
              this.error = 'You do not have enough token to join this contest';
              this.tag = 'danger';
            } else {
  ...
  joinGameInit(_CONTEST, _LINEUP, _CURRENTBALANCE, _LINEUPLIST, _USERBALANCE) {
    ...
    this.confirmJoinForm.patchValue({ lineup: this.joinGameInitObj.lineuplist });
  ...
  validateTeam(playerType, playerId) {
    ...
    const maxPlayersPerTeam = this.finalSelectedPlayersArr.filter((obj) => obj.team_abbreviation === playerDetail.team_abbreviation);
  ...
  resetSelectedPlayers = () => {
    ...
    if (this.lineupPage === 'add') this.selectedPlayerData = [];
    this.defendersArray = [];
    this.midfieldersArray = [];
    this.forwardsArray = [];
    this.goalkeepersArray = [];
  ...
  fillPlayGround() {
    ...
    // this.lineupPage==='edit'? console.log('calling getAllRoster in fillPlayerGround'):'';
        ...
        (response: Response) => {
          if (response["response_code"] === 200) {
            ...
            if (this.lineupPage === "edit") {
              // this.fillPlayGround();
              this.getUsersLineUp();
              //console.log('calling getAllRoser in getLineupMaterData')
              this.getAllRosters();
            } else {
              //console.log('calling getAllRoser in getLineupMaterData')
              this.getAllRosters();
          ...
          .subscribe(
            response => {
              ...
              if (response.response_code === 200) {
                ...
                setTimeout(() => this.router.navigate([this.sports_id + "/my-league/upcoming"]), 4000);
  ...
  confirmJoinContest(lineup?) {
    ...
    if (this.contest.contest_access_type === '3') {
      ...
      this.service
        ...
        .subscribe(response => {
          ...
          this.tag = 'success'; this.error = response.message; this.isLoading = false;
