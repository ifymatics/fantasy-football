...
export class EditProfileComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.editForm  = new FormGroup({
      ...
      'master_country_id': new FormControl( null),//this.profileDetail.master_country_id
...
getCounty() {
 this.countryList = [];
  const reqParams = {};
  this.service.api('user/my_profile/get_country_list', reqParams, 'post', this.session)
  .subscribe((response) => {
      if (response.response_code === 200) {
         console.log(response);
          response = response.data;
...
updateProfile() {
  console.log(this.editForm.value.phone_no);
