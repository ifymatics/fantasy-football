...
export class JoinContestComponent implements OnInit, OnDestroy {
  @ViewChild("firstThingsModal", { static: true })
  firstThingsModal: ModalDirective;
  @ViewChild("joinGameConfirmModal", { static: true })
  joinGameConfirmModal: ModalDirective;
  @ViewChild("subscriptionModal", { static: true })
  subscriptionModal: ModalDirective;
  @ViewChild("contestinfo", { static: false }) contestinfo: ModalDirective;
  toJoinContest;
  confirmJoinForm;
  message;
  tag;
  ...
  league_selected;
  joinGameInitObj;
  subscriptionObj = { amount: 1000, token: 8 };
  ...
  lsObjPosting = false;
  mobileCarouselImage = [];
  webSideTopCarouselImage = [];
  webSideBottomCarouselImage = [];
  lsObj = {
    header_center: {},
    right_top: {},
    right_bottom: {},
  ...
  ngOnInit() {
    ...
    if (this.utilityService.checkLocalStorageStatus("user")) {
      this.currentUser = this.utilityService.getLocalStorage(
        "user"
      ).data.user_profile;
      this.league = this.utilityService.getLocalStorage("league");
      this.session = this.utilityService.getLocalStorage("user")
  ...
  getmobileCarouselImage() {
    this.mobileCarouselImage = [];
  }
  ...
  LobbyMasterData() {
    ...
    this.service
      .api(
        "fantasy/lobby/get_lobby_master_data",
        this.data,
        "post",
        this.session
      )
  ...
  getContestSeason(collection, offset?) {
    ...
    const param = {
      sports_id: 5,
      collection_master_id: collection.collection_master_id
      ...
      offset: offset || "0",
      league_id: collection.league_id
        ? collection.league_id
    ...
    this.service
      .api(
        "fantasy/lobby/get_contests_of_collection",
        param,
        "post",
        this.session
      )
      .subscribe(
        result => {
          ...
          if (!offset) {
            ...
            this.contestList = this.contestList.concat(
              result["data"].contest_list
            );
          }
  ...
  getLobbyMatches(league) {
    ...
    this.service
      .api("fantasy/lobby/get_lobby_matches", param, "POST", this.session)
      .subscribe(
        response => {
  ...
  getFeatureContest(league_id) {
    ...
    this.service
      .api(
        "fantasy/lobby/get_lobby_feature_contest",
        reqParams,
        "POST",
        this.session
      )
  ...
  get_sponsor_images() {
    ...
    this.service
      .api(
        "fantasy/league_sponsor/league_sponsor_list",
        reqParams,
        "POST",
        this.session
      )
      .subscribe(
        response => {
          if (response.response_code === 200) {
            this.rightBtmSponList = this.utilityService.filterArr(
              response.data.advertisements,
              "ls_position_id",
              1
            );
            this.rightTopSponList = this.utilityService.filterArr(
              response.data.advertisements,
              "ls_position_id",
              2
            );
            this.HeaderCenterSponList = this.utilityService.filterArr(
              response.data.advertisements,
              "ls_position_id",
              3
            );
            this.HeaderSliderSponList = this.utilityService.filterArr(
              response.data.advertisements,
              "ls_position_id",
              4
            );
            this.league_detail = {
              background_image: league_selected.background_image
            };
            ...
            this.sponsorIntervals.push(
              setInterval(this.updateSponsorBanner, 5000)
            );
          }
  ...
  updateSponsorBanner = () => {
    ...
    if (user) {
      const rightBtmSponList = this.rightBtmSponList[
        Math.floor(Math.random() * this.rightBtmSponList.length)
      ],
        rightTopSponList = this.rightTopSponList[
          Math.floor(Math.random() * this.rightTopSponList.length)
        ],
        HeaderCenterSponList = this.HeaderCenterSponList[
          Math.floor(Math.random() * this.HeaderCenterSponList.length)
        ];
  ...
  checkProfileComplete() {
    ...
    if (
      !userDetail.data.user_profile.email ||
      !userDetail.data.user_profile.first_name
    ) {
  ...
  // run_timer (id, currentTimestamp, endTimesStamp){
  //   if(!id || !currentTimestamp || !endTimesStamp){
  //       return false;
  //     }
  //    let eventTime= endTimesStamp; //1495051200 Timestamp - Wed, 17 Apr 2017 19:58:00 GMT
  //    let currentTime = currentTimestamp; //1495051080 Timestamp - Wed, 17 Apr 2017 20:00:00 GMT
  //    let diffTime = eventTime - currentTime;
  //    let duration = diffTime*1000;
  //    let interval = 1000;
  //     clearInterval(coundowntimer[id]);
  ...
  //    let coundowntimer[id] = setInterval(function(){
  //     duration = moment.duration(duration - interval, 'milliseconds');
  //     var hours = (duration.hours()<=9) ? (0+''+duration.hours()) : duration.hours();
  //     var minutes = (duration.minutes()<=9) ? (0+''+duration.minutes()) : duration.minutes();
  //     var seconds = (duration.seconds()<=9) ? (0+''+duration.seconds()) : duration.seconds();
  //     //var timer = duration.hours() + ":" + duration.minutes() + ":" + duration.seconds();
  //     timerVal[id] = hours + ":" + minutes + ":" + seconds;
  //      // console.log(vm.timerVal[id]);
  //       angular.element(".timer_"+id).text(timerVal[id]);
  //         if(timerVal[id]=='00:00:00' || endTimesStamp < currentTimestamp ){
  //             angular.element(".timer_"+id).text('00:00:00');
  //             clearInterval(coundowntimer[id]);
  //         }
  //     }, interval);
  // }
  getContestDetail(contest) {
    // vm.selectedCollection.is_all_league_selected = true;
    let param = {
      contest_id: contest.contest_id,
      contest_unique_id: contest.contest_unique_id
    };
    this.service
      .api("fantasy/contest/get_contest_detail", param, "POST", this.session)
      .subscribe(
        response => {
          console.log(response);
          response.data.money_bag = contest.money_bag;
          response.data.currentUser = this.currentUser;
          this.currentContest = response.data;
          this.contestinfo.show();
        },
        error => {
          console.log(error);
          if (error["error"]["global_error"] === "Session key has expired") {
  ...
  checkUserSubscription(contest, string, contestType) {
    ...
    if (contest.contest_access_type === "2") {
      ...
      this.service
        .api("user/finance/get_user_subscribed", param, "POST", this.session)
        .subscribe(response => {
          //console.log(response);
          if (response.response_code === 200) {
            ...
            param.sub_amt = 250;
            const txt =
              "THE SUBSCRIPTION FEE WILL BE DEDUCTED FROM YOUR ACCOUNT BALANCE IF YOU HAVE ENOUGH CASH?";
    ...
    //    // end of subscription check
      ...
      // .subscribe(
      // (response) => {
      if (this.user_balance) {
        // console.log(response);
        //  const user_balance =  response.data.user_balance,
        const point_balance = parseFloat(this.user_balance.point_balance),
          entry_fee = parseFloat(contest.entry_fee),
          token = +this.user_balance.token;
        if (contest.contest_access_type === "3") {
          // console.log(entry_fee, token)
          if (entry_fee > token) {
            this.tag = "danger";
            this.joinButtonDisabled = false;
            this.contestId = "";
            this.message = `You only have ${token} token, but you need ${entry_fee} token to join this game, please get more token to continue!!`;
          } else {
        ...
        // this.utilityService.userBalance.emit( 'its working' );
          ...
          this.tag = "danger";
          this.message = "You dont have enough cash";
          this.joinButtonDisabled = false;
          this.contestId = "";
          // alert('You dont have enough cash');
          // this.notEnoughCashInit(contest.entry_fee, user_balance, contest);
  ...
  joinContest(contestData) {
    ...
    this.service
      .api("fantasy/lobby/get_user_lineup_list", param, "post", this.session)
      .subscribe(
        response => {
          if (response["response_code"] === 200) {
  ...
  fromFirstThingsModal() {
   ...
   this.service.interComponetsTalks( this.toJoinContest, 'contest');
         ...
         500);*/
    this.lobbyservice.toFirstThingFirst(
      this.toJoinContest,
      this.lineupList,
      ""
    );
  ...
  joinGame(contestData, lineup, lineupList) {
    ...
    if (contestData.is_turbo_lineup === "1") {
      const isTurboAvail = this.utilityService.filterArr(
        lineupList,
        "is_turbo_lineup",
        contestData.is_turbo_lineup
      );
      const isTurboTypeAvail = this.utilityService.filterArr(
        lineupList,
        "turbo_lineup_type",
        contestData.turbo_contest_type
      );
      ...
      const isRegAvail = this.utilityService.filterArr(
        lineupList,
        "is_turbo_lineup",
        contestData.is_turbo_lineup
      );
  ...
  confirmJoinContest(lineup?, forToken?) {
    this.isLoading = true;
    ...
    if (
      this.joinGameInitObj.contest.contest_access_type === "3" ||
      this.joinGameInitObj.contest.contest_access_type === 3
    ) {
      this.service
        .api("fantasy/contest/join_premium_game", param, "POST", this.session)
        .subscribe(
          response => {
            ...
            this.router.navigate([
              "/" + this.sports_id + "/" + this.league.league_id + "/my-league"
            ]);
